...
def settings():
    """ Окно настройки для объективов"""

    settings = Tk()
    settings.title("Settings Lenses")
    Times = font.Font(family="Times", size=12)

    # Формируем подписи полей объективов

    LAB1 = Label(settings, font=Times, text="Lens 1")
    LAB2 = Label(settings, font=Times, text="Lens 2")
    LAB3 = Label(settings, font=Times, text="Lens 3")
    LAB4 = Label(settings, font=Times, text="Lens 4")
    LAB5 = Label(settings, font=Times, text="Lens 5")

    # Формируем подписи полей для ввода диаметра поля зрения

    LAB1_ = Label(settings, font=Times, text="Diameter pole")
    LAB2_ = Label(settings, font=Times, text="Diameter pole")
    LAB3_ = Label(settings, font=Times, text="Diameter pole")
    LAB4_ = Label(settings, font=Times, text="Diameter pole")
    LAB5_ = Label(settings, font=Times, text="Diameter pole")

    # Формируем поля ввода названия объективов
    ...
    ).pack(side=TOP, fill=BOTH, padx=5, pady=5, expand=1)

    Button(settings, font=Times, text="Exit", command=settings.destroy).pack(
        side=TOP, fill=BOTH, padx=5, pady=5, expand=1
    )
    settings.mainloop()
...
def param(collection, thinsection, obj, uch, chk_state, chk_state1, chk_state2, chk_state3):
    ...
    ready(
        path=collection.get(),
        pattern="*.jpg",
        thinsection_name=thinsection.get(),
        lense_name=obj.get(),
        uch_name=uch.get(),
        two_circle=two_circle_,
        two_square=two_square_,
